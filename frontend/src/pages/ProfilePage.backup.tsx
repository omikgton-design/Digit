...
import { createAdvisory, createArticle, createSoftware, deleteAdvisory, deleteArticle, deleteSoftware, getAdvisories, getArticleDetail, getArticles, getAdvisoryDetail, getSoftwareDetail, getSoftwares, updateAdvisory, updateArticle, updateSoftware } from "../api/website";
            ...
            <div className="profile-card">
              <button className="profile-avatar profile-avatar-button" type="button" onClick={() => fileInputRef.current?.click()}>
                {profile.logo_url ? (
                  ...
                  <span className="profile-avatar-placeholder">{(profile.name || "D").slice(0, 1)}</span>
                )}
              ...
              <input ref={fileInputRef} className="profile-logo-input" type="file" accept="image/*" onChange={handleLogoPick} />
              <h3>{profile.name}</h3>
              <p className="profile-role">{profile.email}</p>

              <div className="profile-action-buttons">
                <button className="profile-action-button" type="button" onClick={() => setModalMode("profile")}>
                  Мэдээлэл шинэчлэх
                </button>
                <button className="profile-action-button" type="button" onClick={() => setModalMode("password")}>
                  Нууц үг солих
                </button>
              </div>
              ...
              <div className="profile-details">
                {profileDetails.map((item) => {
                  const rawValue = profile[item.value];
                  const value = typeof rawValue === "string" && rawValue.trim() ? rawValue : "-";
                  return (
                    <div className="profile-detail-row" key={item.value}>
            ...
            <div className="profile-card profile-card-main">
              <div className="profile-tabs">
                {tabItems.map((item) => (
                  <button
                    key={item.id}
                    className={`profile-tab ${item.id === activeTab ? "active" : ""}`}
                    type="button"
              ...
              {activeTab === "overview" ? (
                ...
                <div className="profile-tab-panel">
                  {"fields" in activeTabItem && activeTabItem.fields.map((field) => {
                    const rawValue = profile[field.value];
                    const value = typeof rawValue === "string" && rawValue.trim() ? rawValue : "-";
                    return (
                      <div className="profile-tab-item" key={field.value}>
